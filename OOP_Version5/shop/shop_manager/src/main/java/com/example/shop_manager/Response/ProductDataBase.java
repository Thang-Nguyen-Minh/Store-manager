...
//        txtId.setEditable(false); // ID tự động tạo
            ...
            public void actionPerformed(ActionEvent e) {
                try (Connection connection = getConnection()) {
                    ...
                    while (!isExistProduct) {
                        String checkProductName = "SELECT * FROM Product WHERE name = ?";
                        try (PreparedStatement statement = connection.prepareStatement(checkProductName)) {
                            statement.setString(1, name);
                            ResultSet rs = statement.executeQuery();
                            if (rs.next()) {
                                isExistProduct = true;
                                JOptionPane.showMessageDialog(table,"This product is already exist. Please add a new one.");
                                name = JOptionPane.showInputDialog(table, "Enter Product Name:");
                            }
    ...
    private void searchProductByName(String searchName) {
        model.setRowCount(0);
        productList.clear();
        try (Connection connection = DatabaseConnection.getConnection()) {
            ...
            if (resultSet.next()) {
                ...
                JOptionPane.showMessageDialog(table, "Customer not found : " + searchName);
            }
        } catch (SQLException e) {
            e.printStackTrace();
