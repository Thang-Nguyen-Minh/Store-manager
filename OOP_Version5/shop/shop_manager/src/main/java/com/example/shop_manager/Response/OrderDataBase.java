...
public class OrderDataBase extends JPanel {
            ...
            try (PreparedStatement checkOrderStmt = conn.prepareStatement(checkOrder)) {
                ...
                try (ResultSet rs = checkOrderStmt.executeQuery()) {
                    if (!rs.next()) {
                        ...
                        JOptionPane.showMessageDialog(this, "Updated Order successfully.");
                    }
                    else{
                ...
                try (ResultSet rs = checkProductStmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        try (PreparedStatement updateOrderPriceStmt = conn.prepareStatement(updateOrderPrice)) {
                            updateOrderPriceStmt.setDouble(1, Double.parseDouble(String.format("%.2f", productPrice*quantity)));
        ...
        if (selectedRow != -1) {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                try (Connection conn = DatabaseConnection.getConnection()) {
                    ...
                    // Xóa dữ liệu trong bảng Report_Order
                ...
                JOptionPane.showMessageDialog(this, "Order deletion was canceled.");
            }
        } else {
    ...
    private void loadData() {
        tableModel.setRowCount(0);  // Xóa tất cả các dòng trong bảng trước khi load lại dữ liệu


        // SQL query: Lấy tất cả các đơn hàng, không phân biệt status
        // SQL query: Lấy tất cả các đơn hàng, không phân biệt status
        String sql = """
SELECT
    o.id AS order_id,
...
""";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                // Làm tròn tổng giá trị đơn hàng tới 2 chữ số sau dấu phẩy
                ...
                Object[] rowData = {
                        ...
                        rs.getDouble("price"),
                        rs.getInt("quantity"),
                        total,   // Hiển thị tổng giá trị đã làm tròn
                        status
                };
